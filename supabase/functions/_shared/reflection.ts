// Pure, dependency-free helpers for the quest-result function.
// Kept apart from index.ts so they can be unit-tested outside Deno
// (index.ts imports Deno + npm:@google/genai, which a plain test runner
// cannot load).

export type Answer = { question: string; answer: string };
export type Reflection = { title: string; body: string; takeaway: string };
...
/** Deterministic fallback used when Gemini is unreachable or misbehaves. */
export function generateReflection(
  ...
  const hasCreative = answerValues.some((a) => ["create", "creative"].includes(a));
  const hasHighScale = answers.some((a) => a.answer === "4" || a.answer === "5");
  const hasLowScale = answers.some((a) => a.answer === "1" || a.answer === "2");
  ...
  if (hasOverthinking) {
    ...
    title = isId ? "Uang Bukan Cermin Nilaimu" : "Money Is Not a Mirror of Your Worth";
  } else if (hasCreative) {
    title = isId
...
/** Validates a parsed Gemini payload; throws on an unusable shape. */
export function assertReflectionShape(parsed: unknown): Reflection {
  const p = parsed as Partial<Reflection> | null;
  if (
